...
function Allproducts() {
  ...
  function filteringProducts(allProducts, selectedFilter, searchWords) {
    ...
    (productData) =>
      productData.name.toLowerCase().includes(searchWords.toLowerCase()) !== -1
  );
    }
    if (selectedFilter) {
      filteredProducts = filteredProducts.filter(
      ...
      <div className="products-container">
        ...
        <div className="right-column">
          <div className="top-row">
            <Search searchQuery={searchQuery} handleSearchInput={handleSearchInput}></Search>
          </div>
          <div className="mid-row">
            <Recommeneded handleRecommendedBtns={handleRecommendedBtns}></Recommeneded>
          </div>
